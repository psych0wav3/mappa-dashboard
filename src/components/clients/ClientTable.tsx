...
export default function ClientTable({ initialData }: { initialData: Client[] }) {
  ...
  // 🔄 mantém estado local mas sincroniza sempre que o servidor mandar lista nova
  ...
  const byStatus = React.useMemo(
    () => rows.filter((c) => (tab === "active" ? c.active !== false : c.active === false)),
    [rows, tab],
  );
    ...
    <div className="space-y-3">
      ...
      <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="flex items-center gap-2">
          ...
          <Button className="btn-brand text-white" onClick={() => router.push("/clients/new")}>
            Novo cliente
          </Button>
        </div>
      </div>
