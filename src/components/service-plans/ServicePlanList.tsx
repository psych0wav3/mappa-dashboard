...
import { CalendarClock, ChevronLeft, ChevronRight, CirclePause, CirclePlay, Plus, Search, X } from "lucide-react";
...
type ServicePlanListProps = {
  ...
  searchQuery: string;
  pending?: boolean;
  getCustomerName: (plan: ServicePlan) => string;
  onSearchChange: (value: string) => void;
  onClearSearch: () => void;
  onCreateFirst: () => void;
  onStatusChange: (plan: ServicePlan, status: "ACTIVE" | "PAUSED") => void;
  onGenerateOrders: (plan: ServicePlan) => void;
};
...
export default function ServicePlanList({
  ...
  function renderAction(plan: ServicePlan) {
      ...
      return (
        <div className="flex flex-wrap items-center justify-end gap-2">
          ...
          <Button
            ...
            size="sm"
            disabled={pending}
            onClick={() => onStatusChange(plan, "PAUSED")}
            className="h-8 rounded-lg px-3 text-xs"
          >
    ...
    if (plan.status === "PAUSED") {
      return (
        <Button
          ...
          size="sm"
          disabled={pending}
          onClick={() => onStatusChange(plan, "ACTIVE")}
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="mt-0.5 text-xs text-slate-500">
                Consulte e gerencie os atendimentos recorrentes.
              </p>
            </div>
          </div>
      ...
      {!hasPlans ? (
        <div className="px-5 py-12 text-center sm:px-6">
          ...
          <p className="mx-auto mt-2 max-w-lg text-sm leading-6 text-slate-500">
            Crie a primeira rotina de atendimento e envie para aprovação do cliente.
          </p>
        ...
        <div className="px-5 py-12 text-center sm:px-6">
          ...
          <p className="mx-auto mt-2 max-w-lg text-sm leading-6 text-slate-500">
            Não encontramos resultados para a pesquisa informada.
          </p>
            ...
            <table className="w-full min-w-[900px] border-collapse">
              <thead>
                <tr className="border-b border-slate-200 bg-slate-50">
                  ...
                  <th className="min-w-56 px-5 py-3 text-right text-[11px] font-semibold uppercase tracking-wide text-slate-500">
                    Ações
                  </th>
                </tr>
              </thead>
                ...
                {paginatedPlans.map((plan, index) => (
                  <tr
                    ...
                    className={[
                      ...
                      index !== paginatedPlans.length - 1 ? "border-b border-slate-100" : "",
                    ].join(" ")}
                  >
