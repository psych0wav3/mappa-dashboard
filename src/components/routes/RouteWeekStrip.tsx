    ...
    <section className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm sm:p-5">
      ...
      <div className="mt-4 grid gap-2 sm:grid-cols-2 lg:grid-cols-7">
        {weekDays.map((day) => {
          ...
          return (
            <button key={day.date} type="button" onClick={() => onSelectDate(day.date)} className={`flex h-[110px] min-w-0 flex-col rounded-xl border px-3 py-3 text-left transition ${selected ? "border-sky-400 bg-sky-50 ring-2 ring-sky-100" : "border-slate-200 bg-white hover:border-sky-200"}`}>
              <div className="flex h-5 min-w-0 items-center justify-between gap-2">
                <span className={`shrink-0 text-xs font-bold ${selected ? "text-sky-700" : "text-slate-600"}`}>
                  {day.short}
                </span>

                <div className="flex min-w-0 items-center justify-end gap-1">
                  {today ? (
                    <span className="shrink-0 rounded-full border border-emerald-200 bg-emerald-50 px-2 py-0.5 text-[9px] font-bold text-emerald-700">
                      Hoje
                    </span>
                  ) : null}

                  {meta.status ? (
                    <span className="shrink-0 rounded-full border border-emerald-200 bg-emerald-50 px-2 py-0.5 text-[9px] font-semibold text-emerald-700">
                      {meta.status}
                    </span>
                  ) : null}
                </div>
              ...
              <div className="mt-1 min-h-[17px] text-[11px] leading-[17px] text-slate-500">
