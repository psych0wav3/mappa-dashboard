...
import type { RouteDayMeta, TechnicianWeekMetric } from "./routeWeek.utils";
...
type RouteTechnicianWeekSelectorProps = {
  ...
  technicianMetrics: Record<string, TechnicianWeekMetric>;

  weekStartDate: string;
  selectedDate: string;
  todayIso: string;

  dayMeta: Record<string, RouteDayMeta>;
  ...
  const summaries = React.useMemo(() => {
    return Object.fromEntries(
      technicians.map((technician) => {
        const metric = technicianMetrics[technician.id] ?? {
          routes: 0,
          orders: 0,
          completed: 0,
        };

        const progress = metric.orders > 0 ? `${metric.completed}/${metric.orders} concluídos` : "Sem atendimentos na semana";

        return [
          technician.id,
          {
            primary: `${metric.orders} ${metric.orders === 1 ? "atendimento" : "atendimentos"} na semana`,
            secondary: `${metric.routes} ${metric.routes === 1 ? "dia com rota" : "dias com rota"}`,
            status: progress,
          },
        ];
      }),
