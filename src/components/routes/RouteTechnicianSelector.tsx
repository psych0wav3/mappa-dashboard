        ...
        <div className="mt-4 max-h-[260px] overflow-y-auto pr-1">
          <div className="grid auto-rows-[122px] gap-3 p-0.5 md:grid-cols-2 xl:grid-cols-4">
            {visibleTechnicians.map((technician) => (
              <TechnicianRouteCard key={technician.id} name={technician.name} selected={technician.id === technicianId} summary={summaries[technician.id] ?? EMPTY_SUMMARY} onClick={() => onSelectTechnician(technician.id)} />
            ))}
          </div>
        </div>
      ) : (
        <div className="mt-4 rounded-xl border border-dashed border-slate-300 bg-slate-50 px-5 py-8 text-center text-sm text-slate-500">Nenhum técnico encontrado.</div>
      )}
    </section>
  );
}