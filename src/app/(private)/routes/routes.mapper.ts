...
function hasValidAddress(order: ApiServiceOrder) {
  if (order.customerAddressId) {
    return true;
  }

  if (order.address && typeof order.address !== "string" && order.address.id) {
    return true;
  }

  if (typeof order.address === "string" && order.address.trim().length > 0 && !order.address.toLowerCase().includes("não informado")) {
    return true;
  }
...
export function normalizeWorkOrderForRoute(order: ApiServiceOrder): AvailableRouteWorkOrder {
  const description = order.description || "";
  const serviceKind = parseServiceKind(description, order.title);
  ...
  const technicianName = parseTechnicianName(description);
  const technicianId = parseTechnicianId(description);
  const lat = getAddressLatitude(order.address);
  const lng = getAddressLongitude(order.address);
  ...
  return {
    ...
    description,
    serviceKind,
    frequencyLabel: parseFrequencyLabel(description),
    weekdays,
    weekdaysLabel,
    scheduledTime,
    scheduledDate: toApiDate(order.scheduledDate || ""),
    address: addressLabel(order.address),
    hasAddress: hasValidAddress(order),
    hasCoordinates: lat !== 0 && lng !== 0,
    status: status === "waitingexecution" ? "WAITING_EXECUTION" : "READY_FOR_ROUTE",
    lat,
    lng,
...
export function normalizeRouteDetails(route: ApiRouteDetailsResponse): RouteDashboardItem {
  const serviceOrders = (route.serviceOrders || [])
    .map((item, index) => {
      ...
      return {
        ...
        scheduledTime: scheduledTime === "Horário não informado" ? null : scheduledTime,
