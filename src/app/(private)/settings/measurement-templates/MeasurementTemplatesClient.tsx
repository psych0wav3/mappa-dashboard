...
export default function MeasurementTemplatesClient({
  ...
  const [fields, setFields] = React.useState<MeasurementTemplateField[]>([]);
  ...
  function openEditForm(template: MeasurementTemplate) {
  ...
  function addField() {
    ...
    const alreadyExists = fields.some(
      (field) =>
        field.fieldName.toLowerCase() === cleanFieldName.toLowerCase(),
    );

    if (alreadyExists) {
      toast.error("Já existe um campo com esse nome técnico no template.");
      return;
    }
    ...
    setFields((current) => [
      ...current,
      {
        ...
        label: cleanLabel,
        fieldType,
        unit: fieldType === "NUMBER" ? fieldUnit.trim() || null : null,
        minValue: 0,
        maxValue: 0,
  ...
  function handleSave() {
    ...
    if (!fields.length) {
      toast.error("Adicione ao menos um campo ao template.");
      return;
    }
    ...
    startTransition(async () => {
      try {
        if (editingTemplate) {
          const updated = await updateMeasurementTemplate({
          ...
          toast.success("Template de medição atualizado com sucesso.");
        } else {
          const created = await createMeasurementTemplate(input);

          setTemplates((current) => [created, ...current]);
          toast.success("Template de medição criado com sucesso.");
        }
        ...
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div>
            ...
            <p className="mt-1 text-sm text-slate-500">
              Configure modelos de medições para uso nas ordens e visitas.
            </p>
          </div>
          ...
          <div className="mb-4 flex items-start justify-between gap-4">
            <div>
              ...
              <p className="text-xs text-slate-500">
                Crie o template com os campos que o técnico deverá preencher.
              </p>
            </div>
          ...
          <div className="mt-5 rounded-xl border border-slate-200 bg-slate-50 p-4">
            <div className="mb-3 flex items-center justify-between gap-3">
              <div>
                ...
                <p className="text-xs text-slate-500">
                  Adicione pH, cloro, alcalinidade ou qualquer medição usada na
                  visita.
                </p>
              </div>
            ...
            <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-5">
              <div className="xl:col-span-2">
                ...
                <Input
                  value={fieldLabel}
                  onChange={(event) =>
                    handleFieldLabelChange(event.target.value)
                  }
              ...
              <div>
                ...
                <select
                  value={fieldRequired ? "REQUIRED" : "OPTIONAL"}
                  onChange={(event) =>
                    setFieldRequired(event.target.value === "REQUIRED")
                  }
                  className="h-10 w-full rounded-md border border-slate-300 bg-white px-3 text-sm outline-none focus:border-sky-400"
                >
                ...
                <tbody>
                  ...
                  {fields.length === 0 && (
                    <tr>
                      ...
                      >
                        Nenhum campo adicionado ao template ainda.
                      </td>
                    </tr>
                  )}
                </tbody>
            ...
            <tbody>
              ...
              {templates.length === 0 && (
                <tr>
                  ...
                  >
                    Nenhum template de medição cadastrado ainda.
                  </td>
                </tr>
              )}
            </tbody>
