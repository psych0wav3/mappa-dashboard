...
function normalizeUserAgreement(
  item: ApiAgreement,
): UserAgreement {
  return {
...
export async function listAgreementsAdmin(
  ...
  return extractItems<ApiAgreement>(
    data,
  ).map(
    normalizeAgreement,
  );
...
export async function createAgreement(
  ...
  if (
    ...
    throw new Error(
      "Informe o conteúdo do termo.",
    );
  }

  const created =
    await mappaFetch<ApiAgreement>(
      "/api/agreements",
      {
        method:
          "POST",
...
export async function acceptAgreement(
  ...
  await mappaFetch<null>(
    `/api/me/agreements/${agreementId}/accept`,
    {
      method:
        "POST",
    },
